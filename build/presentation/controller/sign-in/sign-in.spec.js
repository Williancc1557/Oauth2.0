...
const makeSut = () => {
    ...
    const sut = new sign_in_1.SignInController(getAccountByEmailStub, resetRefreshTokenStub, encrypterStub, validationStub);
    return {
        sut,
...
describe("SignIn Controller", () => {
    test("should SignIn returns statusCode 400 if account don't exists", async () => {
        ...
        expect(res).toStrictEqual((0, http_helper_1.badRequest)(new errors_1.UserNotExistsError()));
    });
    ...
    test("should return statusCode 400 if encrypter.compare return false", async () => {
        ...
        const res = await sut.handle(makeFakeHttpRequest());
        expect(res).toStrictEqual((0, http_helper_1.badRequest)(new errors_1.InvalidParamError("password")));
    });
